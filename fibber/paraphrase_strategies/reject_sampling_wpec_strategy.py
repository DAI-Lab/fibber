...
def accept_or_reject(tokenizer, origin, batch_tensor, pos, previous_idxs,
                     candidate_idxs, logpdf, use_metric, weight, eps=0.9):
    ...
    unnormalized_log_p_candidate = -weight * P_SMOOTHING * np.maximum(
        eps - np.asarray(use_semantic_similarity), 0)

    alpha = np.exp(unnormalized_log_p_candidate)
    accept = np.asarray(np.random.rand(len(alpha)) < alpha, dtype="int32")
    idxs = candidate_idxs * accept + previous_idxs * (1 - accept)
    return idxs

class RejectSamplingWPECStrategy(StrategyBase):

    def __init__(self, FLAGS, metric_bundle):
        """Initialize the strategy."""
        super(RejectSamplingWPECStrategy, self).__init__(FLAGS, metric_bundle)
    ...
    def _parallel_sequential_generation(self, seed):
        ...
        for ii in range(self._max_iter):
            ...
            idxs_prime = accept_or_reject(
                ...
                logpdf_joint, self._use_metric, 1, eps=0.7 if (ii <= self._burnin) else 0.9)
